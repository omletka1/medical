...
$this->title = 'Журнал приема медикаментов';
$this->params['breadcrumbs'][] = $this->title;
?>
<style>
    /* Основные стили страницы */
    .medication-logs-index {
        padding: 30px 0;
    ...
    .medication-logs-index h1::after {
        content: '';
        position: absolute;
        bottom: -10px;
        left: 0;
...
<div class="medication-logs-index">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <?= Html::a('Добавить запись', ['create'], ['class' => 'btn btn-create-log']) ?>
        ...
        <?= GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'label' => 'Медикамент',
                    'value' => function($model) {
                        return $model->medication->name ?? 'Не указано';
                    },
                ],
                [
                    'attribute' => 'dosage',
        ...
        <?php foreach ($dataProvider->getModels() as $model): ?>
            <div class="card mb-4">
                <div class="card-header">
                    Запись #<?= Html::encode($model->id) ?>
                </div>
                <div class="card-body">
                    <p><strong>Пользователь:</strong> <?= Html::encode($model->user->username ?? 'Не указано') ?></p>
                    <p><strong>Медикамент:</strong> <?= Html::encode($model->medication->name ?? 'Не указано') ?></p>
                    <p><strong>Дозировка:</strong> <?= Html::encode($model->dosage) ?></p>
                    <p><strong>Время приема:</strong> <?= Yii::$app->formatter->asDatetime($model->time_taken) ?></p>
                </div>
